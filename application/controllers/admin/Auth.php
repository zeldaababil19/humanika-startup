		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember)) {
				...
				if ($this->session->userdata('usertype') == '5') {
					...
					$this->data['message'] = "<div class='col-lg-12'><div class='alert alert-block alert-warning'><i class='ace-icon fa fa-bullhorn green'></i> Anda tidak punya akses sebagai admin</div></div>";
